...
\begin{lyxcode}
\emph{\footnotesize /{*}~Esta~é~uma~função~vararg~que~recebe~como~argumentos~adicionais~uma~lista}{\footnotesize \par}

~\emph{\footnotesize ~~terminada~em~NULL~de~PyObjects~a~serem~passados~à~função~Python~indicada}{\footnotesize \par}

~\emph{\footnotesize ~~no~primeiro~argumento.~{*}/}{\footnotesize \par}
...
~{\footnotesize ~~.tp\_name~=~\char`\"{}point\char`\"{},~~~~~~~~~~~~}~\emph{\footnotesize /{*}~O~nome~da~classe~{*}/}{\footnotesize \par}

~{\footnotesize ~~.tp\_basicsize~=~sizeof(point),~}~\emph{\footnotesize /{*}~O~tamanho~da~área~de~memória~a~ser~alocada~{*}/}{\footnotesize \par}

~{\footnotesize ~~.tp\_getattr~=~point\_getattr,~~~}~\emph{\footnotesize /{*}~A~função~de~acesso~a~atributos~{*}/}{\footnotesize \par}

~{\footnotesize ~~.tp\_flags~=~Py\_TPFLAGS\_DEFAULT~}~\emph{\footnotesize /{*}~Classe~não~requer~tratamentos~especiais~{*}/}{\footnotesize \par}
...
~\emph{\footnotesize ~~/{*}~Ruby~pode~chamar~esta~função~com~o~mesmo~objeto~nos~dois~parâmetros;}{\footnotesize \par}

~\emph{\footnotesize ~~~~~nesse~caso,~ignore~a~chamada~e~retorne~o~objeto~{*}/}{\footnotesize \par}
...
\begin{lyxcode}
{\footnotesize rb\_define\_method(point\_class,~\char`\"{}initialize\_copy\char`\"{},~point\_initialize\_copy,~1);}{\footnotesize \par}
...
{\footnotesize rb\_iv\_set(obj,~\char`\"{}c\char`\"{},~g);}~\emph{\footnotesize /{*}~Equivale~a:~rb\_ivar\_set(obj,~rb\_intern(\char`\"{}c\char`\"{}),~g);~{*}/}{\footnotesize \par}
...
}. Há funções para realizar a cópia de arrays (\texttt{Get}/\texttt{Set}\emph{<tipo}>\texttt{\-ArrayRegion}),
...
\emph{\footnotesize ele~será~uma~variável~opaca~(seu~conteúdo~não~será~acessível~via~Perl).~{*}/}{\footnotesize \par}
...
~\emph{\footnotesize ~~/{*}~Obviamente~seria~mais~eficiente~armazenar~os~valores~em~variáveis}{\footnotesize \par}

~\emph{\footnotesize ~~~~~temporárias~em~C,~mas~obtenhamos~os~dois~últimos~valores~da}{\footnotesize \par}

~\emph{\footnotesize ~~~~~seqüência~de~volta~do~array~Perl~para~fins~de~ilustração:~{*}/}{\footnotesize \par}
...
~{\footnotesize ~~jstring~s~=~({*}J)->NewString(J,~NULL,~0);~~~~}~\emph{\footnotesize /{*}~s~=~\char`\"{}\char`\"{}~{*}/}{\footnotesize \par}

~{\footnotesize ~~int~len~=~({*}J)->GetArrayLength(J,~a);~~~~}~\emph{\footnotesize ~~~/{*}~len~=~a.length~{*}/}{\footnotesize \par}

~{\footnotesize ~~for~(int~i~=~0;~i~<~len;~i++)~\{}{\footnotesize \par}

~{\footnotesize ~~~~~jobject~o~=~({*}J)->GetObjectArrayElement(J,~a,~i);}~\emph{\footnotesize ~~~~~/{*}~o~=~a{[}i{]}~{*}/}{\footnotesize \par}

~{\footnotesize ~~~~~jstring~os~=~({*}J)->CallObjectMethod(J,~o,~toString);}~\emph{\footnotesize ~~/{*}~os~=~o.toString()~{*}/}{\footnotesize \par}

~{\footnotesize ~~~~~jstring~s2~=~({*}J)->CallObjectMethod(J,~s,~concat,~os);}~\emph{\footnotesize /{*}~s2~=~s.concat(os)~{*}/}{\footnotesize \par}
...
{\footnotesize VALUE~val\_result~=~rb\_funcall(Qnil,~teste,~2,~rb\_str\_new2(\char`\"{}entrada\char`\"{}),~INT2NUM(2));}{\footnotesize \par}
...
{\footnotesize VALUE~resultado~=~rb\_iterate(chama\_uma\_funcao\_ruby,~Qnil,~um\_bloco\_C,~Qnil);}{\footnotesize \par}
\end{lyxcode}
...
\begin{lyxcode}
{\footnotesize Call}\textrm{\emph{\footnotesize <tipo><retorno>}}{\footnotesize Method}\textrm{\emph{\footnotesize <argumentos>}}{\footnotesize \par}
\end{lyxcode}


O \emph{<tipo>} pode ser \texttt{Static} para funções estáticas, passando
na chamada uma \texttt{jclass} como parâmetro; \texttt{Nonvirtual}
...
é indicado em \emph{<retorno>}: \texttt{Void}, \texttt{Object}, \texttt{Int},
etc.
...
{\footnotesize long~retorno~=~({*}J)->CallStaticIntMethod(J,~exemplo,~teste,~entrada,~(jint)2);}{\footnotesize \par}
\end{lyxcode}
...
{\footnotesize long~retorno~=~lua\_tointeger(L,~-1);}~\emph{\footnotesize /{*}~Obtém~o~resultado~no~topo~da~pilha~(-1)~{*}/}{\footnotesize \par}
...
da classe>}\texttt{\_}\emph{<nome do método>}. No caso de sobrecarga
de funções, um sufixo é adicionado indicando o tipo dos parâmetros
de entrada (por exemplo, \texttt{Java\_Classe\_metodo\_\_DI} para
a versão de \texttt{Classe.metodo} que aceita um \texttt{double} e
um \texttt{int} como parâmetros).
...
~{\footnotesize ~~const~char{*}~entrada~=~luaL\_checkstring(L,~1);}~\emph{\footnotesize /{*}~Obtém~o~primeiro~parâmetro~{*}/}{\footnotesize \par}

~{\footnotesize ~~long~n~=~luaL\_checkinteger(L,~2);~~~~~~~~~~~~}~\emph{\footnotesize /{*}~Obtém~o~segundo~parâmetro~{*}/}{\footnotesize \par}

~{\footnotesize ~~printf(\char`\"{}Recebi~\%s~e~\%ld~\textbackslash{}n\char`\"{},~entrada,~n);}{\footnotesize \par}

~{\footnotesize ~~lua\_pushinteger(L,~42);~~~~~~~~~~~~~~~~~~~~~~}~\emph{\footnotesize /{*}~Empilha~o~valor~de~retorno~{*}/}{\footnotesize \par}

~{\footnotesize ~~return~1;~~~~~~~~~~~~~~~~~~~~~~~~~~~~}~\emph{\footnotesize /{*}~Retorna~um~valor,~do~topo~da~pilha~{*}/}{\footnotesize \par}
...
~{\footnotesize ~~char{*}~entrada~=~script\_get\_string(env,~0);~}~\emph{\footnotesize /{*}~Entrada,~índice~0:~string~{*}/}{\footnotesize \par}

~{\footnotesize ~~int~n~=~script\_get\_int(env,~1);~~~~~~~~~~~~}~\emph{\footnotesize /{*}~Entrada,~índice~1:~inteiro~{*}/}{\footnotesize \par}
...
~{\footnotesize ~~script\_put\_int(env,~0,~42);~~~~~~~~~~~~~~~~}~\emph{\footnotesize /{*}~Retorno,~índice~0:~inteiro~{*}/}{\footnotesize \par}
...
Como comentado na seção anterior, a função \texttt{script\_plugin\_init\_}\emph{<linguagem>}
...
para \texttt{script\_plugin\_init\_}\emph{<linguagem>}, para que o
plugin possa fazer chamadas à biblioteca principal, e um identificador
...
\begin{lyxcode}
{\footnotesize lua\_pushstring(L,~\char`\"{}LibScript.env\char`\"{});}~\emph{\footnotesize ~~/{*}~Empilha~o~índice~{*}/}{\footnotesize \par}

{\footnotesize lua\_pushlightuserdata(L,~env);~~~~~~~}~\emph{\footnotesize /{*}~Empilha~o~ambiente~LibScript~{*}/}{\footnotesize \par}

{\footnotesize lua\_settable(L,~LUA\_REGISTRYINDEX);~~}~\emph{\footnotesize /{*}~registro{[}\char`\"{}LibScript.env\char`\"{}{]}~=~env~{*}/}{\footnotesize \par}
\end{lyxcode}
...
\emph{\footnotesize /{*}~Cria~o~módulo.~O~primeiro~parâmetro~é~o~nome~do~módulo,}{\footnotesize \par}

~\emph{\footnotesize ~~o~segundo~a~lista~de~métodos~do~módulo,~que~será~inicialmente~vazio.~{*}/}{\footnotesize \par}
...
via C como \texttt{class} \emph{<Nome>} via Ruby geram um erro indicando
que o valor já foi definido com outro tipo. Como Ruby possui classes
abertas, uma construção \texttt{class} \emph{<Nome>} para um \emph{<Nome>}
já existente é entendida como a continuação da descrição da classe,
e não como a redefinição de \emph{<Nome>}. Python, por sua vez, não
disponibiliza recursos na API para a descarga de módulos, mas permite
atribuir \texttt{None} à global referente ao módulo. O módulo pode
ser importado novamente, mas a mesma instância dele, armazenada internamente
por Python, será retornada. A seguinte sessão interativa de linha
de comando permite observar este comportamento, que ocorre tanto diretamente
em Python como via a API de C:
...
~\emph{\footnotesize ~~/{*}~Decrementa~o~contador~de~estados,~uma~variável~global}~{\footnotesize static}~\emph{\footnotesize do~plugin.~{*}/}{\footnotesize \par}
...
Na implementação do plugin LibScript-Lua, a função \texttt{script\_lua\_stack\_to\_buffer}
...
a partir de C, \texttt{script\_plugin\_call\_lua}, usa a função \texttt{script\_lua\_stack\_to\_buffer}
para armazenar no buffer LibScript os valores de retorno da função
Lua invocada, já que estes são retornados na pilha virtual. Quando
o código Lua chama funções implementadas em C ou em outro plugin,
\texttt{script\_lua\_stack\_to\_buffer} é usada para converter os
parâmetros de entrada da função, também recebidos na pilha virtual.
...
~{\footnotesize ~~nargs~=~lua\_gettop(L);~~~}~\emph{\footnotesize /{*}~Número~de~elementos~na~pilha~de~Lua~{*}/}{\footnotesize \par}
...
~{\footnotesize ~~int~len~=~script\_buffer\_len(env);}~\emph{\footnotesize /{*}~Número~de~elementos~no~buffer~{*}/}{\footnotesize \par}
...
~{\footnotesize ~~~~~~~~s~=~script\_get\_string(env,~i);}~\emph{\footnotesize /{*}~A~string~pertence~ao~chamador.~{*}/}{\footnotesize \par}

~{\footnotesize ~~~~~~~~lua\_pushstring(L,~s);}{\footnotesize \par}

~{\footnotesize ~~~~~~~~free(s);}~\emph{\footnotesize /{*}~Libera~a~string,~já~que~Lua~armazena~sua~própria~cópia.~{*}/}{\footnotesize \par}
...
~{\footnotesize ~~int~len~=~PyTuple\_GET\_SIZE(tuple);}~\emph{\footnotesize /{*}~Número~de~elementos~da~tupla~{*}/}{\footnotesize \par}

~{\footnotesize ~~script\_reset\_buffer(env);~~~~~~~~~}~\emph{\footnotesize /{*}~Esvazia~o~buffer~LibScript~{*}/}{\footnotesize \par}

~{\footnotesize ~~for~(i~=~0;~i~<~len;~i++)~\{}{\footnotesize \par}

~{\footnotesize ~~~~~PyObject{*}~o~=~PyTuple\_GET\_ITEM(tuple,~i);}~\emph{\footnotesize /{*}~Obtém~elemento~da~tupla~{*}/}{\footnotesize \par}

~{\footnotesize ~~~~~script\_python\_put\_object(env,~i,~o);~~~~~}~\emph{\footnotesize /{*}~Insere-o~no~buffer.~{*}/}{\footnotesize \par}
...
{\footnotesize args~=~script\_python\_buffer\_to\_tuple(env);~}~\emph{\footnotesize /{*}~Obtém~o~parâmetros~de~entrada~{*}/}{\footnotesize \par}

{\footnotesize ret~=~PyEval\_CallObject(func,~args);}~\emph{\footnotesize ~~~~~~~/{*}~Chama~uma~função~Python~{*}/}{\footnotesize \par}

\emph{\footnotesize /{*}~...~{*}/}{\footnotesize \par}

{\footnotesize if~(ret~==~Py\_None)~~~~~~~~~~~~~~~~~~~~~~~~}~\emph{\footnotesize /{*}~Se~a~função~não~retornou~valor~{*}/}{\footnotesize \par}

~{\footnotesize ~~script\_reset\_buffer(env);~~~~~~~~~~~~~~~}~\emph{\footnotesize /{*}~Apenas~zere~o~buffer~LibScript~{*}/}{\footnotesize \par}

{\footnotesize else~if~(PyTuple\_Check(ret))~~~~~~~~~~~~~~~}~\emph{\footnotesize /{*}~Se~retornou~uma~tupla~{*}/}{\footnotesize \par}

~{\footnotesize ~~script\_python\_tuple\_to\_buffer(env,~ret);}~\emph{\footnotesize /{*}~Insira~seus~elementos~no~buffer~{*}/}{\footnotesize \par}

{\footnotesize else~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~}~\emph{\footnotesize /{*}~Se~retornou~outro~tipo~de~objeto~{*}/}{\footnotesize \par}

~{\footnotesize ~~script\_python\_put\_object(env,~0,~ret);~~}~\emph{\footnotesize /{*}~Insira-o~como~único~elemento~{*}/}{\footnotesize \par}
\end{lyxcode}
...
\begin{lyxcode}
{\footnotesize script\_python\_tuple\_to\_buffer(env,~args);~~~}~\emph{\footnotesize /{*}~Obtém~o~parâmetros~de~entrada~{*}/}{\footnotesize \par}

{\footnotesize err~=~script\_call(env,~fn\_name);~~~~~~~~~~~~}~\emph{\footnotesize /{*}~Chama~um~função~via~LibScript~{*}/}{\footnotesize \par}

{\footnotesize /{*}~...~{*}/}{\footnotesize \par}

{\footnotesize switch(script\_buffer\_len(env))~\{}{\footnotesize \par}

{\footnotesize case~0:~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~}~\emph{\footnotesize ~/{*}~Se~a~função~não~retornou~valor~{*}/}{\footnotesize \par}

~{\footnotesize ~~Py\_RETURN\_NONE;~~~~~~~~~~~~~~~~~~~~~~~~~~}~\emph{\footnotesize ~/{*}~Retorne~o~valor~Python~'None'~{*}/}{\footnotesize \par}

{\footnotesize case~1:~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~}~\emph{\footnotesize ~/{*}~Se~retornou~um~único~valor~{*}/}{\footnotesize \par}

~{\footnotesize ~~return~script\_python\_get\_object(env,~0);~}~\emph{\footnotesize ~/{*}~Converta~e~retorne-o~{*}/}{\footnotesize \par}

{\footnotesize default:~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~}~\emph{\footnotesize /{*}~Se~retornou~mais~de~um~valor~{*}/}{\footnotesize \par}

~{\footnotesize ~~return~script\_python\_buffer\_to\_tuple(env);}~\emph{\footnotesize /{*}~Retorne-os~em~uma~tupla~{*}/}{\footnotesize \par}
...
~{\footnotesize ~~\char`\"{}bootstrap~LibScript;\char`\"{}~~}~\emph{\footnotesize /{*}~Inicializa~o~módulo~de~extensão~{*}/}{\footnotesize \par}

~{\footnotesize ~~\char`\"{}package~\%s;\char`\"{}}~\emph{\footnotesize ~~~~~~~~~~~/{*}~Declara~o~pacote~do~ambiente~{*}/}{\footnotesize \par}

~{\footnotesize ~~\char`\"{}\$LibScript::env~=~\%p;\char`\"{}~}~\emph{\footnotesize /{*}~Armazena~o~ponteiro~do~ambiente~em~Perl~{*}/}{\footnotesize \par}

~{\footnotesize ~~\char`\"{}sub~AUTOLOAD~\{\char`\"{}}{\footnotesize \par}

~{\footnotesize ~~~~~\char`\"{}our~\$AUTOLOAD;\char`\"{}}{\footnotesize \par}

~\emph{\footnotesize ~~~~~/{*}~Extrai~o~nome~do~método~do~nome~qualificado~{}``pacote::método''~{*}/}{\footnotesize \par}
